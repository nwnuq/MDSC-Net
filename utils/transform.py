...
def ISIC2018_transform(sample, train_type):
    ...
    if train_type == 'train':
        image, label = randomcrop(size=(256, 256))(image, label)
        image, label = randomflip_rotate(image, label, p=0.5, degrees=30)
    else:
        image, label = resize(size=(256, 256))(image, label)

    image = ts.Compose([ts.ToTensor(),
                        ts.Normalize(mean=(0.71227854, 0.549649, 0.5084288), std=(0.15962751, 0.16466813, 0.17912234))])(image)
